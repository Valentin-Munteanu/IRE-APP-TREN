...
<head>
    ...
    <title>Formularul pentru 4 persoane în vagonul 4</title>
...
<body>
    ...
    <div class="login_div">
        <h4>Vagonul:  {{$bilet->vagon_number4}} </h4>
            <h4>Clasa: {{$bilet->class2c}}C</h4>

            <div id="selected-date">
                <h4>Data: {{$selectedDate}}</h4>
            </div>
            <p>Locul selectat: {{ $selectedSeats4 }}</p>
            <p>Pret: {{$totalPrice4}} MDL</p>
       ...
       <div class="all-log">
        <h4>Completează formularul pentru 3 persoane</h4>
        <div class="login_form">
            <div class="auth_form">



                <form action="{{ route('form4store', ['numSeets4' => $numseets4, 'bilet_id' => $bilet->id, "selectedDate" => $selectedDate, "selectedSeats4" => $selectedSeats4, "totalPrice4" => $totalPrice4]) }}" method="POST">
                    @csrf
    <label for="">Pasager 1</label>
                    ...
                    <input type="hidden" name="selectedSeats4" value="{{ $selectedSeats4 }}">
                @error("selectedSeats4")
                    <p>{{$message}}</p>
                @enderror
                    <input type="hidden" name="selectedDate" value="{{ $selectedDate }}">
                    <input type="hidden" name="totalPrice4" value="{{ $totalPrice4}}">
