...
class PassagersVagon2Request extends FormRequest
{
    /**
    ...
    public function rules(): array
    {
        return [
            ...
            'selectedSeats2' => [
                'required',
                Rule::unique('passagers_vagon2s')->where(function ($query) {
                    return $query->where('biletes_id', $this->bilet_id)
                                 ->where('selectedSeats2', $this->input('selectedSeats2'));
                })->ignore($this->id),
            ]
        ];
    }
    ...
    {
        // Mesajele de eroare pentru toate persoanele
        return [
            ...
            'selectedSeats2' => 'Locul selectat este deja ocupat.'
